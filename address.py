...
# --- 1. 카카오 API 통신 함수 (로그 반환 구조로 변경하여 누락 방지) ---
...
def get_coords_only(address, api_key, session):
    ...
    def add_log(api_type, query, status_result):
        logs.append({
            "단계": "2단계", "호출 시간": datetime.datetime.now().strftime("%H:%M:%S.%f")[:-3],
            "API 종류": api_type, "요청 파라미터": str(query), "응답 결과": status_result
        })
...
def get_driving_distance(start_coord, end_coord, api_key, session):
    ...
    def add_log(api_type, status_result):
        logs.append({
            "단계": "2단계", "호출 시간": datetime.datetime.now().strftime("%H:%M:%S.%f")[:-3],
            "API 종류": api_type, "요청 파라미터": query_str, "응답 결과": status_result
        })
...
if df_raw is not None:
    ...
    # [Step 1.5] 정정 결과 요약표 & 수동 보정 UI (Step 2에서도 계속 보여줍니다)
    if st.session_state.step >= 1:
        ...
        with chart_col:
            ...
            fig1.update_layout(margin=dict(t=40, b=0, l=0, r=0), height=200, showlegend=False)
        ...
        if len(error_indices) == 0:
            ...
            for idx in error_indices:
                ...
                with col_chk:
                    # step 2일 때는 비활성화(disabled)할 수도 있지만, 그냥 두어도 무방합니다.
        ...
        # --- 사이드바: 진행 상황판 및 2단계 실행 버튼 (Step 1일 때만 표시) ---
    ...
    # [Step 2] 최종 거리 산출 (Step 1 아래에 이어짐)
    if st.session_state.step == 2:
        st.markdown("---")
        st.subheader("📊 2단계 완료: 최종 구간 거리 산출 결과")
    ...
    if st.session_state.api_log:
        ...
        with col_log1:
            with st.expander(f"📌 1단계 주소 검증 API 이력 ({cnt_1}건)", expanded=False):
                if not log_step1.empty:
                    st.dataframe(log_step1.drop(columns=['단계']), use_container_width=True)
                else:
                    st.info("기록된 이력이 없습니다.")
                    
        with col_log2:
            with st.expander(f"📌 2단계 거리 산출 API 이력 ({cnt_2}건)", expanded=False):
                if not log_step2.empty:
                    st.dataframe(log_step2.drop(columns=['단계']), use_container_width=True)
                else:
                    st.info("기록된 이력이 없습니다.")

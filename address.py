        ...
        if st.session_state.step == 1:
            ...
            if len(error_indices) == 0:
                ...
                for idx in error_indices:
                    ...
                    # [수정] 카카오맵 검색 버튼 추가 및 레이아웃 조정
                    col_addr, col_btn1, col_btn2, col_chk = st.columns([5, 2, 2, 1.5])
                    with col_addr: st.code(orig_addr, language="plaintext")
                    with col_btn1: st.link_button("🔍 구글 검색", f"https://www.google.com/search?q={urllib.parse.quote(orig_addr)}", use_container_width=True)
                    with col_btn2: st.link_button("🗺️ 카카오맵", f"https://map.kakao.com/?q={urllib.parse.quote(orig_addr)}", use_container_width=True)
                    with col_chk:

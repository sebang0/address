...
thread_local = threading.local()  # [수정] 스레드별 독립 세션 관리


def get_thread_session() -> requests.Session:
    """스레드마다 독립적인 requests.Session을 반환합니다.

    [수정] requests.Session을 여러 스레드가 공유하면 urllib3 연결 풀에서
    Race Condition이 발생할 수 있습니다. thread_local을 통해 스레드별로
    세션을 분리하여 이 문제를 방지합니다.
    """
...
API_TIMEOUT = 10   # 주소 검색 타임아웃 (초)
DIST_TIMEOUT = 15  # 경로 탐색 타임아웃 (초)
...
def get_address_info(address, api_key):
    ...
    [수정] 세션을 파라미터로 받지 않고 get_thread_session()으로 스레드별 세션을 사용합니다.
    [수정] 모든 session.get() 호출에 timeout=API_TIMEOUT을 추가했습니다.
    [수정] 네트워크 오류/타임아웃에 대한 예외처리를 추가했습니다.
    """
    logs = []
    ...
    if api_key == "SIMULATION_MODE":
        if "포승읍" in addr_str:
            ...
            return (
                "모호함(다중검색)", "경기 평택시 포승읍", "126.8", "36.9",
                ["경기 평택시 포승읍 도곡리", "경기 평택시 포승읍 내기리", "경기 평택시 포승읍 원정리"],
                logs,
            )
    ...
    session = get_thread_session()  # [수정] 스레드별 세션
    headers = {"Authorization": f"KakaoAK {api_key}"}
    url = "https://dapi.kakao.com/v2/local/search/address.json"

    try:
        res = session.get(url, headers=headers, params={"query": addr_str}, timeout=API_TIMEOUT)  # [수정] timeout 추가
    ...
    except requests.exceptions.Timeout:  # [수정] 타임아웃 예외처리
        add_log("1단계", "주소 검색", addr_str, "타임아웃")
        return "검색 불가", "", None, None, [], logs
    except Exception as e:  # [수정] 기타 네트워크 오류
        add_log("1단계", "주소 검색", addr_str, f"에러: {e}")
        return "검색 불가", "", None, None, [], logs
    ...
    if len(parts) >= 2:
        ...
        try:
            fb_res = session.get(kw_url, headers=headers, params={"query": fallback_query}, timeout=API_TIMEOUT)  # [수정]
            if fb_res.status_code == 200 and fb_res.json().get("documents"):
                ...
                cands = list(set(filter(None, [doc.get("address_name") for doc in fb_docs])))[:3]
                if cands:
                    return "검색 불가(유사추천)", "", None, None, cands, logs
            else:
                add_log("1단계", "유사 검색(폴백)", fallback_query, "실패")
        except requests.exceptions.Timeout:  # [수정]
            add_log("1단계", "유사 검색(폴백)", fallback_query, "타임아웃")
        except Exception as e:  # [수정]
            add_log("1단계", "유사 검색(폴백)", fallback_query, f"에러: {e}")

    return "검색 불가", "", None, None, [], logs


def get_coords_only(address, api_key):
    """수동 정정된 주소의 좌표를 재조회합니다.
    ...
    if not address or address in ["확인 불가", "보정 제외"]:
    ...
    try:
        ...
        add_log("재조회(주소)", address, "타임아웃")
        return None, None, logs
    except Exception as e:
        add_log("재조회(주소)", address, f"에러: {e}")
        return None, None, logs
    ...
    try:
        ...
        add_log("재조회(키워드)", address, "타임아웃")
    except Exception as e:
        add_log("재조회(키워드)", address, f"에러: {e}")

    add_log("재조회(전체)", address, "완전 실패")
    return None, None, logs


def get_driving_distance(start_coord, end_coord, api_key, car_type):
    """주행 거리를 산출합니다.

    [수정] 세션을 파라미터로 받지 않고 get_thread_session()으로 스레드별 세션을 사용합니다.
    [수정] timeout=DIST_TIMEOUT 추가 및 Timeout 예외처리 추가.
    """
    logs = []
    ...
    try:
        res = session.get(url, headers=headers, params=params, timeout=DIST_TIMEOUT)  # [수정]
...
if "validation_skipped" not in st.session_state:  # [수정] 명시적 플래그 추가
    st.session_state.validation_skipped = False


def reset_step():
    st.session_state.step = 0
    st.session_state.api_log = []
    st.session_state.validation_skipped = False  # [수정]
...
MAX_WORKERS = 5
col_start = col_end = col_ton = None
...
if df_raw is not None and st.session_state.step == 0:
    st.info("👈 좌측 사이드바에서 설정을 확인한 후 작업 방식을 선택해주세요.")
    ...
    if unmapped_tons:
        st.warning(
            f"⚠️ **업데이트되지 않은 신규 톤급(또는 문자) {len(unmapped_tons)}건의 차종은 1종으로 반영됩니다.**\n"
            f"(대상: {', '.join(map(str, list(unmapped_tons)[:10]))} 등)"
        )
    else:
...
if df_raw is not None:
    ...
    if st.session_state.step == 0:
        ...
        if st.sidebar.button("1️⃣ 주소 정정 후 거리 산출", type="primary", use_container_width=True):
            ...
            unique_addrs = [str(addr) for addr in unique_addrs if str(addr).strip() and str(addr).lower() != "nan"]
            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                # [수정] session 파라미터 제거 (스레드 내부에서 get_thread_session() 사용)
                future_to_addr = {
                    executor.submit(get_address_info, addr, api_key): addr
                ...
                for future in as_completed(future_to_addr):
                    ...
                    try:  # [수정] 스레드 내부 예외 포착
                        ...
                        status, correct_addr, x, y, cands, logs = "검색 불가", "", None, None, [], []

                    with log_lock:  # [수정] 로그 병합 시 락 사용
        ...
        # 버튼 2: 주소 정정 없이 바로 거리 산출
        if st.sidebar.button("2️⃣ 주소 정정 없이 바로 거리 산출", type="secondary", use_container_width=True):
            st.session_state.api_log = []
            st.session_state.validation_skipped = True  # [수정] 명시적 플래그

            unique_addrs = pd.unique(df_raw[[col_start, col_end]].astype(str).values.ravel("K"))
            unique_addrs = [str(addr) for addr in unique_addrs if str(addr).strip() and str(addr).lower() != "nan"]

            mock_mapping = [
                {"원본 주소": addr, "정정된 주소": addr, "상태": "검증 생략", "제외여부": False, "X": None, "Y": None}
                for addr in unique_addrs
            ]
            ...
            st.session_state.status_map = {addr: "검증 생략" for addr in unique_addrs}
            st.session_state.df_raw = df_raw
            st.session_state.step = 2
            st.rerun()

    # ── 1단계: 주소 검증 결과 화면 ────────────────
    # [수정] status_map 첫 번째 키 조회 대신 명시적 플래그(validation_skipped)로 판단
    if st.session_state.step >= 1 and not st.session_state.get("validation_skipped", False):
        ...
        with dash_col:
            ...
            df_summary["출발지 검증"] = df_summary[col_start].astype(str).map(
                lambda x: st.session_state.status_map.get(x, "확인 불가")
            )
            df_summary["도착지 검증"] = df_summary[col_end].astype(str).map(
                lambda x: st.session_state.status_map.get(x, "확인 불가")
            )
            ...
            def highlight_verification(row):
                ...
                for i, col_name in enumerate(row.index):
                    if col_name in ["출발지 검증", "도착지 검증"]:
                        ...
                        elif "추천" in val or "모호함" in val:
                            styles[i] = "background-color: #fff0b3; color: #996600; font-weight: bold"
                        elif "정상" in val:
                            styles[i] = "background-color: #e6ffe6; color: #006600"
                return styles

            st.dataframe(
                df_summary[[col_start, col_end, "출발지 검증", "도착지 검증"]].style.apply(
                    highlight_verification, axis=1
                ),
                column_config={
                    col_start: st.column_config.TextColumn(width=150),
                    col_end: st.column_config.TextColumn(width=150),
                },
        ...
        if st.session_state.step == 1:
            ...
            if len(error_indices) == 0:
                ...
                for idx in error_indices:
                    ...
                    with col_btn:
                        st.link_button(
                            "🔍 구글 검색",
                            f"https://www.google.com/search?q={urllib.parse.quote(orig_addr)}",
                            use_container_width=True,
                        )
                    with col_chk:
                    ...
                    if is_excluded:
                        ...
                        default_idx = (
                            options.index(current_val)
                            if current_val and current_val in options
                            else (len(options) - 1 if current_val and current_val != "선택 안 함" else 0)
                        )
                        choice = st.radio(
                            "변경할 주소 선택",
                            options,
                        ...
                        if choice == "직접 입력":
                            new_addr = st.text_input(
                                "새 주소 입력",
                                value=current_val if default_idx == len(options) - 1 else "",
                                key=f"text_{idx}",
                                label_visibility="collapsed",
                            )
                            if new_addr.strip():
            ...
            if len(error_indices) == 0 or completed_tasks == len(error_indices):
                ...
                if uncompleted_rows:
                    st.sidebar.caption("미완료 행: " + ", ".join(uncompleted_rows[:8]))
    ...
    if st.session_state.step == 2:
        ...
        if "final_df" not in st.session_state:
            ...
            addr_dict = {}
            mapping_rows = list(st.session_state.mapping_df.iterrows())
            coords_total = len(mapping_rows)
            coords_done = 0

            # ── 좌표 재조회 대상 분류 ─────────────
            coords_to_fetch = []  # (원본 주소, 정정된 주소)
            for _, row in mapping_rows:
                if row.get("제외여부", False):
                    ...
                    x, y = row["X"], row["Y"]
                    # [수정] pd.isna(x) → x is None (API 좌표는 문자열로 반환되므로 명시적 체크)
                    if final_addr and final_addr != "확인 불가" and x is None:
                        coords_to_fetch.append((row["원본 주소"], final_addr))
                    else:
                        addr_dict[row["원본 주소"]] = (final_addr, x, y)
                        coords_done += 1

            # ── 좌표 재조회 병렬 처리 ─────────────
            # [수정] 기존 순차 처리 → ThreadPoolExecutor 병렬 처리
            if coords_to_fetch:
                status_text.text(f"1/3. 좌표 확인 및 변환 중... (0/{len(coords_to_fetch)})")
                with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                    coord_futures = {
                        executor.submit(get_coords_only, final_addr, api_key): (orig_addr, final_addr)
                        for orig_addr, final_addr in coords_to_fetch
                    }
                    for future in as_completed(coord_futures):
                        orig_addr, final_addr = coord_futures[future]
                        try:  # [수정] 예외처리
                            x, y, logs = future.result()
                            with log_lock:
                                st.session_state.api_log.extend(logs)
                        except Exception:
                            x, y = None, None
                        addr_dict[orig_addr] = (final_addr, x, y)
                        coords_done += 1
                        prog.progress(coords_done / coords_total)
                        status_text.text(f"1/3. 좌표 확인 및 변환 중... ({coords_done}/{coords_total})")
            else:
                prog.progress(coords_done / max(coords_total, 1))
                status_text.text(f"1/3. 좌표 확인 및 변환 중... ({coords_done}/{coords_total})")

            df_target = st.session_state.df_raw.copy()
            out_cols = ["적용 차종", "정정_출발지", "정정_도착지", "주행거리(km)", "산출 비고", "카카오맵 자동길찾기"]
            df_target_clean = df_target.drop(
                columns=[c for c in out_cols if c in df_target.columns]
            ...
            # ── 경로 거리 병렬 산출 ────────────────
            # [수정] 좀비 ThreadPoolExecutor(429~431행) 완전 삭제 후 단일 executor만 사용
            route_cache = {}
            if routes_to_fetch:
                ...
                with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                    futures = {
                        executor.submit(get_driving_distance, s, e, api_key, c_type): (s, e, c_type)
                        for s, e, c_type in routes_to_fetch
                    }
            ...
            for _, row in df_target_clean.iterrows():
                ...
                if s_info[1] and e_info[1] and "보정 제외" not in [s_info[0], e_info[0]]:
                    ...
                    links.append(
                        f"https://m.map.kakao.com/scheme/route?"
                        f"sp={s_info[2]},{s_info[1]}&ep={e_info[2]},{e_info[1]}"
                        f"&by=car&carType={kakao_cartype}"
                    )
                else:
                    links.append(None)
        ...
        with msg_col:
            st.success(
                f"📌 **산출 완료!** 전체 **{total_cnt}건** 중 "
                f"정상 산출 **{success_cnt}건** / 실패 **{fail_cnt}건**"
            )
        with pie_col:
            fig2 = px.pie(
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "카카오맵 자동길찾기": st.column_config.LinkColumn(
                    "카카오맵 자동길찾기", display_text="🚀 즉시 경로 확인"
                ),
            },
            use_container_width=True,
        )
        ...
        st.download_button(
            ...
            file_name="route_distances_final.xlsx",
    ...
    if st.session_state.api_log:
        st.markdown("---")
        with st.expander(
            f"🛠️ 시스템 디버그: API 호출 이력 (총 {len(st.session_state.api_log)}건)", expanded=False
        ):
